 ...
 *
 * mark_as_dropped_only - instead of deleting the row, mark it as dropped.
...
chunk_tuple_delete(TupleInfo *ti, DropBehavior behavior, bool preserve_chunk_catalog_row)
{
	...
	if (form.compressed_chunk_id != INVALID_CHUNK_ID)
	{
		...
		if (compressed_chunk != NULL)
			ts_chunk_drop(compressed_chunk,
						  behavior,
						  DEBUG1,
						  false /* never preserve catalog rows for compressed chunks */);
	}
...
void
ts_chunk_drop(Chunk *chunk, DropBehavior behavior, int32 log_level, bool preserve_catalog_row)
{
	ObjectAddress objaddr = {
						...
						int32 log_level)
{
	...
	for (; i < num_chunks; i++)
	{
		...
		ts_chunk_drop(chunks + i,
					  (cascade ? DROP_CASCADE : DROP_RESTRICT),
					  log_level,
					  has_continuous_aggs &&
						  cascades_to_materializations == CASCADE_TO_MATERIALIZATION_FALSE);
	}
