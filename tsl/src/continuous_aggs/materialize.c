...
static void
range_clamp_end(InternalTimeRange *range, int64 end_value, int64 bucket_width)
{
	...
	Assert(end_value == ts_time_bucket_by_type(bucket_width, end_value, range->type));
	if (end_value < range->start)
	{
		/* make an empty range */
		range->start = 0;
		range->end = 0;
